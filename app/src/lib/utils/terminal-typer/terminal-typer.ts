...
import { WaitEffect, DeleteEffect, Effect, InsertEffect, EffectInterval, DeleteEffectInterval } from "./effects";
import { Cursor, CursorOptions } from "./cursor";
import { EffectBuilder, CharContainers } from "./types";
import { RequiredOptions } from "~/lib/types/utils";
import { escapeRegex, merge, throwExp } from "~/lib/utils";
...
export class TerminalTyper {
	static createOptions(options?: TerminalTyperOptions): RequiredOptions<TerminalTyperOptions> {
		const intervals: RequiredOptions<TerminalTyperIntervals> = merge(
			{
				insert: {
					variance: "random",
					min: 25,
	...
	private options: RequiredOptions<TerminalTyperOptions>;
